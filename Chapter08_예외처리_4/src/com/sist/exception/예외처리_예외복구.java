 ...
 *                 ----- 한개로 통합 (최상위 예외클래스를 이용한다)
 *                       -------- Exception / Throwable
 *                                ---------   ---------
 *                                예외만 가능    예외+에러
 *                 ----- 순서가 존재 => 작은 예외클래스 ~ 큰 예외클래스
 *                       -------- 계층도 (클래스 크기가 정해진다)
 *                       
 *                       Throwable
 *                           |
 ...
 *          -------- 변수선언 / 클래스 객체 선언   => try~catch 문 안에서 공통적으로 사용할 수 있다
 *          try
 *          {
 *              문장 3
 *              -------------------- if
 ...
 *              }
 *              ** try~catch를 주로 사용
 *         static => 변수, 메서드 => 컴파일시에 메모리에 저장     
 *         instance => 변수, 메서드
 *                     객체가 생성이 되면 => 메모리에 저장    
 *                      
 *         => 메서드() throws 예외클래스, 예외클래스, 예외클래스....
 *                   -------------------------------------
 *                      => 순서는 상관없다
 *                      컴파일러에 알려준다 (예외 선언)
 *                      --------------
 ...
 *                                    예외처리 후에 사용
 *            => 형식)
 *              [접근지정어] [제어어] 리턴형 메서드명(매개변수..) throws 예외클래스1, 예외클래스2...
 *                                                            ----------------------
 *                                                            예상되는 예외 등록                       
 *              => 처리 메서드() throws 예외클래스1, 예외클래스2...
 *              => 처리 메서드()
 *                 try
 *                 {
 *                 }catch()
