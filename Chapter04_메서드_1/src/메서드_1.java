 ...
 * 
 * 		메서드 => 사용할때 (호출)
 * 				호출이 되면 메서드 처음부터 수행	=> 다시 호출된 위치로 복귀
 * 
 * 		메서드 구성요소
 ...
 * 						=> plus(), minus() ...
 * 						=> 변수 식별자와 동일
 * 							1) 알파벳, 한글	=> 알파벳 대소분자 구분
 * 							2) 숫자 사용 가능	=> 앞에 사용할 수 없다
 * 							3) 키워드는 사용이 불가능
 * 							4) 특수문자 사용	=> $, _
 * 							5) 공백이 있으면 안된다
 ...
 * 	=> 메서드: void일 경우에 생략을 하면 자동 첨부
 * 
 * 	메서드는 반드시 호출을 해야 사용이 가능
 * 	void diplay()
 * 	int display()
 * 	int display(int a)
...
public class 메서드_1 {
	...
	public static void main(String[] args) {
		...
		gugudan();
		return;	// 컴파일 시에 return 이 없는 경우에는 자동으로 생성
		// return	=> 메서드 종료
	}
