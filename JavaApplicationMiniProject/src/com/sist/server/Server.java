 ...
 *              {
 *                  // 클래스 간의 공유가 많다 (변수, 메서드)
 *                  // waitVc의 내용을 마음대로 가져다 쓸 수 있다
 *              }
 *          }  
 *          ---------------------------------- 서버 / 빅데이터  
 */
public class Server implements Runnable{
    ...
    public Server()
    {
        try
        {
            ...
            // 대기상태 listen()
            // 단점 : 50명만 접속이 가능
            // new ServerSocket(100, PORT) // 100명까지 가능하게 확장
    ...
    public void run()
    {
        while(true)
        {
            try 
            {
                ...
                //      접속한 경우 => 클라이언트가 접속 시에 호출되는 특수한 메서드
    ...
    public static void main(String[] args) {
        ...
        new Thread(server).start(); // 접속처리 => run() 메서드 호출
    }
    // 같은 서버에서 같은 PORT면 서버는 한번만 수행
    // 통신 담당 클래스
    class Client extends Thread
    {
        // Client의 모든 정보
        ...
        // => 개인별로 전송
        // synchronized => 동기화 (보내고 받은 것을 확인)
        // => async function : 속도가 빠르다, 동시처리 => AJAX (Async JavaScript And XML)
        // => sync funcion : 안정적임, 하나씩 순서대로 처리
