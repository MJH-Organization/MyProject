...
/**
 *		1. 주의점
 *			1) static 메서드에서는 인스턴스변수, 인스턴스 메서드를 사용할 수 없다
 *				=> 사용시에는 객체 생성후에 사용이 가능 
 *			2) static 메서드, static 변수는 this를 사용할 수 없다
 *			3) static 블록에는 static 변수만 초기화가 가능
 *			4) 인스턴스 메서드에서는 인스턴스변수, static변수를 사용할 수 있다. 
 *			5) 초기화 =>	생성자를 이용하는 이유 
 *						---- this, 인스턴스, static 
 ...
 *					생성자, 메서드
 *	------------------------------------------------------------------------------------------
 *	private			멤버변수, 생성자		자신의 클래스 내부				클래스 외부에서는 사용이 불가능
 *					메서드
 *	------------------------------------------------------------------------------------------
 *	=> 크기
 ...
 *	*** 많이 사용
 *		멤버변수	=> 은닉화	private
 *		생성자, 클래스, 메서드	=> 모든 클래스 연결 : public
 *	--------------------- 	다른 클래스에서 사용
 *							--------
 *							다른 폴더에 존재할 수 있다
 *		제어어
 *		=> static | abstract | final
 *		static =>	공통적인 =>	공통으로 사용되는 메서드가 / 변수
 *		final =>	마지막 =>	변수(상수), 메서드(종단), 클래스(종단 클래스)
 *										------------------------
 *										| 있는 그대로 사용
 ...
 *		메서드	: public / default / private / protected
 *				=> 메서드의 역할
 ...
 *				가정 =>	메모리에서 보이지 않는다
 *				=> 어떻게 통신
 *				=> 변수 : 쓰기(메모리 저장), 읽기(메모리에서 데이터 가지고오기)
 *						=> 메서드로 처리:	getter/setter
 *		지역변수	: 접근지정어가 필요없다 (메서드안에서만 사용이 가능)
 *					=> 상수 (final)
 *
 *		*** 객체지향의 3대 특성 (권장)
 *			1. 캡슐화 :	외부로부터 데이터 보호
